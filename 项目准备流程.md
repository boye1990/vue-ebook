...
- 解析+渲染
  ...
  2. 添加动态路由，创建动态组件，将动态组件引入父组件。
  3. 通过动态路由地址，获取需要渲染的电子书名，拼接成nginx的资源链接。
  4. 通过epubjs渲染电子书
- 字体+字号
- 主题
- 阅读进度
...
2. vuex+mixin
3. vue-i18n国际化 
  - npm i --save vue-i18n 安装依赖
  - 在src目录添加lang文件夹，放入语言.js文件， 并新建index.js文件
  - 在main.js中引入上一步创建的js文件，并使用。
  - 通过$t()来使用要国际化的字段。
  - 具体代码才看源代码
4. 动态切换样式+书签手势操作
